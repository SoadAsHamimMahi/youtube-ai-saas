...
/**
 * Enhanced Relative Time with Seconds/Minutes/Hours countdown precision
 */
function getRelativeTime(dateString: string) {
  ...
  if (diffInMs < 0) return 'just now'; // Futures are now
...
export default function DashboardPage() {
  ...
  const [timeUpdate, setTimeUpdate] = useState(0); // Trigger re-renders for relative time
  ...
  /**
   * Update relative times every 5 seconds for a "live" feel
   */
  useEffect(() => {
  ...
  const handleRunNow = async (agentId: string) => {
    ...
    try {
      ...
      if (result.success) {
        await fetchAgents(false); // Refresh silently
      } else {
    ...
    <div className="space-y-10" suppressHydrationWarning>
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-500 font-medium">Manage your AI-powered YouTube monitoring agents.</p>
        </div>
        <button
      ...
      <section className="space-y-6 pb-20">
        ...
        {loading && agents.length === 0 ? (
          ...
          <div className="glass rounded-3xl p-16 text-center space-y-4 border border-dashed border-border-glass">
            ...
            <p className="text-gray-500 max-w-sm mx-auto">Create your first agent to start receiving daily AI-curated YouTube reports!</p>
            <button
              onClick={() => setIsModalOpen(true)}
